...
const lastnames = names.map((n, i) => {
  let namearr = n.split(" ");
  return namearr[namearr.length - 1];
});
console.log(`LastNames are ${lastnames}`);

const rightFormat1 = /^\w+ \w+$/;
let firstlast = names.filter((name) => {
  return name.match(rightFormat1);
});
console.log(`Formatted names are ${firstlast}`);
...
names.forEach((n, i) => {
  titlecasewords.push(
    n
      .split(" ")
      .map((w, i) => w.charAt(0).toUpperCase() + w.substring(1).toLowerCase())
      .join(" ")
  );
});
console.log(`Title Case words are ${titlecasewords}`);

let processednames = names.filter(
  (name) =>
    firstlast.includes(name) && name[name.length - 1].toLowerCase() != "z"
);
processednames.forEach((filt, i) => {
  let titlecased = filt
    .split(" ")
    .map((w, i) => w.charAt(0).toUpperCase() + w.substring(1).toLowerCase())
    .join(" ");
...
processednames.map((enemy, i) => console.log(`Hey ${enemy}, Please sign up.`));
///////////////////////////////////////////////////////////////////////////////
